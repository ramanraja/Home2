/*-------------------------------------------------------------------------------------------------------------
   ...
   code from Boot3
   New in this version:  
      - F() macro for all string constants
      - Separate pins.h to customize hardware pins
      - Disable DHT22 and other sensors with preporcessor directives
      - Soft AP has unique name attaching the MAC id
   ...
   TODO: some web interface (extend WiFi Manager?) to see/edit config.txt and upload to SPIFF
   TODO: Remotely get/set individual config items through MQTT 
   ---------------------------------------------------------------------------------
