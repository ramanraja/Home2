...
//------------------------------------------------------------------------------------------------
AWS::AWS() {
  ////SERIAL_PRINTLN("AWS object created."); there is no serial port when this object is created!
}
...
bool AWS::init_time_client() {
    SERIAL_PRINTLN("Connecting to Time Server...");
    timeClient.begin();
...
bool AWS::init_file_system() {
    bool result = true;
    SERIAL_PRINTLN("Mounting the file system...");
    print_heap();
    if (!SPIFFS.begin()) {
