...
bool Hardware::read_sensors() {
    ...
    sensor_reading_count++;  // increments every minute
    ...
    sensor_data.hindex = dht.computeHeatIndex(tem, hum, false); // here the default is Farenheit !  (verify)
    sensor_data.light = light;
    sensor_data.pir_hits = 0;  // TODO: implement these
    sensor_data.radar_hits = 0;    
    
    pCmd->send_data();  // &sensor_data);
    tem = 0.0f;
    hum = 0.0f;
    light = 0;
    // TODO: reset pir_hits and radar_hits also !
    
    return true;  // true = the main class should operate the lights 
} 
...
void Hardware::relay_on (short relay_number) {
  ...
  pCmd->send_status();  // TODO: send the status as an argument
}
