...
#ifdef ENABLE_DEBUG
void update_started() {
    SERIAL_PRINTLN ("[CallBack] OTA update started.");
}
void update_finished() {
    SERIAL_PRINTLN ("[CallBack] OTA update finished!");
}
void update_progress(int cur, int total) {
    SERIAL_PRINT ("[CallBack] OTA update progress (bytes): ");
    SERIAL_PRINT (cur);
    SERIAL_PRINT (" of ");
    SERIAL_PRINTLN (total);
}
void update_error(int err) {
    SERIAL_PRINT ("[CallBack] OTA update failed. error code: ");
    SERIAL_PRINTLN (err);
}
#endif
//---------------------------------------------------------------------------------
...
int OtaHelper::check_version() {
    ...
    HTTPClient client;
    if (!client.begin(url)) {
        SERIAL_PRINTLN ("--- Malformed version check URL ---");
        ///client.end();
        return (BAD_URL);
    }
    ...
    if(httpCode > 200 && httpCode <= 308) { // code 304 = content unmodified; TODO: revisit!
        SERIAL_PRINTLN ("Cannot  check version: content possibly moved.");
...
int OtaHelper::update_firmware() {
    ...
    SERIAL_PRINT("Response code from FW server: ");
    ...
    switch (result_code) {
        ...
        case HTTP_UPDATE_FAILED:
            ...
            SERIAL_PRINT("HTTP update FAILED. Error msg code = ");
